...
CM_NODEBUG CM_INLINE void cm_dpas_check_common() {
  ...
  constexpr unsigned ops_per_channel = get_ops_per_channel(src1_precision,
                                                           src2_precision);
  ...
  CM_STATIC_ERROR(N1 == ((src1_precision_bits * systolic_depth * ops_per_channel * N) / (repeat_count * sizeof(T1) * 8)),
    "invalid size for Src1");
  
  constexpr unsigned src2_precision_bits = get_precision_bits(src2_precision);
  CM_STATIC_ERROR(N2 == ((src2_precision_bits * systolic_depth * ops_per_channel * repeat_count) / (sizeof(T2) * 8)),
...
cm_dpas(vector<T, N> src0, vector<T1, N1> src1, vector<T2, N2> src2,
        int flag = _GENX_NOSAT) {
  cm_dpas_check_common<src1_precision, src2_precision,
                       systolic_depth, repeat_count, T,
                       T1, T2, N, N1, N2>();

  cm_dpas_check_types<src1_precision, src2_precision,
                      T0, systolic_depth, repeat_count, T,
                      T1, T2, N, N1, N2>();
    ...
    int flag = _GENX_NOSAT)
{
  ...
  vector<T1, N1> _Src1 = src1;
  vector<T2, N2> _Src2 = src2;
  vector<T, N> _Result = details::__cm_intrinsic_impl_dpas_nosrc0<src1_precision, src2_precision, systolic_depth, repeat_count, T, T1, T2, N>(NULL, _Src1, _Src2);
  if (flag != _GENX_SAT)
    return _Result;
    ...
    int flag = _GENX_NOSAT)
{
  ...
  constexpr unsigned ops_per_channel = get_ops_per_channel(src1_precision, src2_precision);
  CM_STATIC_ERROR(ops_per_channel != 0xFFFFFFFF,
    "invalid combination of Src1/Src2 precision");
  ...
  constexpr unsigned src2_precision_bits = get_precision_bits(src2_precision);
  CM_STATIC_ERROR(N2 == ((src2_precision_bits * systolic_depth * ops_per_channel * ((repeat_count + 1)/2)) / (sizeof(T2) * 8)),
    "invalid size for Src2");
  ...
  vector<T, N> _Result =
      details::__cm_intrinsic_impl_dpasw<src1_precision, src2_precision,
                         systolic_depth, repeat_count, T>(_Src0, _Src1, _Src2);
  if (flag != _GENX_SAT)
    return _Result;
    ...
    int flag = _GENX_NOSAT)
{
  ...
  vector<T1, N1> _Src1 = src1;
  vector<T2, N2> _Src2 = src2;
  vector<T, N> _Result = details::__cm_intrinsic_impl_dpasw_nosrc0<src1_precision, src2_precision, systolic_depth, repeat_count, T, T1, T2, N>(NULL, _Src1, _Src2);
  if (flag != _GENX_SAT)
    return _Result;
