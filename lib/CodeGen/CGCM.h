...
public:
  ...
  CodeGen::CMBuiltinKind getCMBuiltinKind(llvm::StringRef Name) const;
  CodeGen::CMBuiltinKind getCMBuiltinKind(const FunctionDecl *F) const {
    llvm::StringRef MangledName = CGM.getMangledName(F);
    return getCMBuiltinKind(MangledName);
  }

  /// \brief Return true if this is a CM builtin direct call.
  bool isCMBuiltinCall(const CallExpr *CE) const {
    const FunctionDecl *F = CE->getDirectCallee();
    return F && getCMBuiltinKind(F) != CMBuiltinKind::CMBK_none;
  }
  bool isCMBuiltin(llvm::StringRef Name) const {
    return getCMBuiltinKind(Name) != CMBuiltinKind::CMBK_none;
  }

  /// \brief Instructrument this call expression or not.
  /// - byref argumment
  /// - intrinsic template translation
  /// - GENX or CALLABLE attribute
  ///
  bool instrumentCMCall(const CallExpr *CE) const {
    assert(CE && "null expression");
    if (isCMBuiltinCall(CE))
      return true;

    const FunctionDecl *FD = CE->getDirectCallee();

    if (FD && (FD->hasAttr<CMGenxMainAttr>() || FD->hasAttr<CMCallableAttr>()))
      return true;

    for (auto Arg : CE->arguments())
      if (Arg->getType()->isCMReferenceType())
        return true;
    return false;
  }
  ...
  public:
    ...
    ~EnterCMCallRAII() { CMRT.popCMCallInfo(); }
  };
  ...
  /// no cast is necessary. Otherwise, returns true and the cast kind is
...
private:
  ...
  llvm::Value *HandleBuiltinLineImpl(CMCallInfo &CallInfo, CMBuiltinKind Kind);
  ...
  /// \brief Postprocess builtin cm_intrinsic_impl_frc.
  llvm::Value *HandleBuiltinFrcImpl(CMCallInfo &CallInfo, CMBuiltinKind Kind);

  /// \brief Postprocess builtin cm_intrinsic_impl_lzd.
  llvm::Value *HandleBuiltinLzdImpl(CMCallInfo &CallInfo, CMBuiltinKind Kind);
  ...
  llvm::Value *HandleBuiltinLrpImpl(CMCallInfo &CallInfo, CMBuiltinKind Kind);

  /// \brief Postprocess builtin cm_pln.
  llvm::Value *HandleBuiltinPlnImpl(CMCallInfo &CallInfo, CMBuiltinKind Kind);
  ...
  /// \brief Postprocess builtin cm_unmask_begin.
  llvm::Value *HandleBuiltinUnmaskBeginImpl(CMCallInfo &CallInfo);

  /// \brief Postprocess builtin cm_unmask_end.
  llvm::Value *HandleBuiltinUnmaskEndImpl(CMCallInfo &CallInfo);
  ...
  /// \brief Postprocess cm_slm_read4.
  void HandleBuiltinSVMRead4Impl(CMCallInfo &Info);

  /// \brief Postprocess cm_slm_write4.
  void HandleBuiltinSVMWrite4Impl(CMCallInfo &Info);
