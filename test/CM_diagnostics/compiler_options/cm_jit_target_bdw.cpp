...
// RUN: %cmc -emit-llvm -Qxcm_jit_target=bdw %s 2>&1 | FileCheck %s

// RUN: %cmc -emit-llvm -Qxcm_jit_target:bdw %s 2>&1 | FileCheck %s

// RUN: %cmc -emit-llvm -Qxcm_jit_targetbdw %s 2>&1 | FileCheck %s

// RUN: %cmc -emit-llvm -Qxcm_jit_target=gen8 %s 2>&1 | FileCheck %s
...
// RUN: %cmc -emit-llvm -Qxcm_jit_target -mcpu=BDW %s 2>&1 | FileCheck %s

// RUN: %cmc -emit-llvm -Qxcm_jit_target -mcpu=Gen8 %s 2>&1 | FileCheck %s

// RUN: %cmc -emit-llvm -march=Bdw -Qxcm_jit_target %s 2>&1 | FileCheck %s

// RUN: %cmc -emit-llvm -Qxcm_jit_target -march=Gen8 %s 2>&1 | FileCheck %s
