...
#define CM_PRINTF_WRITE_8BYTE(dv, scalar)                                           \
    ...
    write(CM_PRINT_BUFFER, offset, header)

#define CM_PRINTF_WRITE_STRING(header, strVect, str, STRING_TYPE)                   \
    ...
    write(CM_PRINT_BUFFER,                     offset,  header);                    \
    write(CM_PRINT_BUFFER, PRINT_HEADER_SIZE + offset, strVect)

template <typename T>
inline _GENX_ uint cm_printf_write_output(uint offset, const T scalar)
{
    vector<uint, 8> header(0);
    header(CM_PRINT_OBJECT_TYPE_ENTRY_INDEX) = CM_PRINT_OBJECT_TYPE_SCALAR;
    header(CM_PRINT_DATA___TYPE_ENTRY_INDEX) = cm_print_datatype(scalar);
...
template <>
inline _GENX_ uint cm_printf_write_output(uint offset, const double scalar)
{
    vector<double, 1> dv = scalar;
    CM_PRINTF_WRITE_8BYTE(dv, scalar);
    return cm_printf_compute_size(scalar);
}

template <>
inline _GENX_ uint cm_printf_write_output(uint offset, const int64_t scalar)
{
...
template <>
inline _GENX_ uint cm_printf_write_output(uint offset, const uint64_t scalar)
{
...
template <>
inline _GENX_ uint cm_printf_write_output(uint offset, const char* str)
{
    CM_PRINTF_WRITE_STRING(header, strVect, str, CM_PRINT_OBJECT_TYPE_STRING);
    return cm_printf_compute_size(str);
}

inline _GENX_ uint cm_printf_write_format(uint offset, const char* str)
{
    CM_PRINTF_WRITE_STRING(header, strVect, str, CM_PRINT_OBJECT_TYPE_FORMAT);
    return cm_printf_compute_size(str);
}
...
template<typename T, typename... Args>
inline _GENX_ uint cm_printf_write_output(uint offset, T value, Args... args)
{
    uint bytesWritten = 0;
    bytesWritten += cm_printf_write_output(bytesWritten + offset, value  );
    bytesWritten += cm_printf_write_output(bytesWritten + offset, args...);
    return bytesWritten;
}

template<typename T, typename... Args>
inline _GENX_ uint cm_printf(T value, Args... args)
{
    vector<uint, 8> offpos(_offset);
    vector<uint, 8> off(0);
    vector<uint, 8> size;
    size(0) = cm_printf_compute_size(value, args...);
    write(CM_PRINT_BUFFER, ATOMIC_ADD, 0, offpos, size, off);

    uint offset = off(0);
    uint bytesWritten = 0;
    bytesWritten += cm_printf_write_format(bytesWritten + offset, value  );
    bytesWritten += cm_printf_write_output(bytesWritten + offset, args...);
    return bytesWritten;
}
...
template<typename T>
inline _GENX_ uint cm_printf(T value)
{
    vector<uint, 8> offpos(_offset);
    vector<uint, 8> off(0);
    vector<uint, 8> size;
    ...
    write(CM_PRINT_BUFFER, ATOMIC_ADD, 0, offpos, size, off);
    return cm_printf_write_format(off(0), value);
}
