...
// This next one is converted to a SIMD1 scattered write and is used to allow writing scalar values
// to buffers
// There is a restriction to 32 bit types for the value written (although SIMD1 scatter does support
// writing less than this - it's more work to implement)
// These are written specifying the scalar payload exactly due to issues with template
// overloading if a generic template type is used instead (too much matches ambiguously)
// The template is used to stop a compilation problem for Linux (matches other overloaded functions)

// We now define our own variant of enable_if so we can create the required templates for atomic operations
// We do this to ensure correctness using all possible compiler variants
namespace hidden {
...
template <typename T>
CM_API extern typename hidden::cm_enable_if<is_fp_or_dword_type<T>::value, bool>::type
write(SurfaceIndex & buf_id, uint global_offset, uint element_offset, T v)
{
    if (workarroundForIpoBugOrFeature) {
        printf("workarroundForIpoBugOrFeature\n");
        vector<uint, 1> _v = v;
        vector<uint, 1> _element_off = element_offset;
        return write(buf_id, global_offset, _element_off, _v);
    }
    else {
        printf("workarroundForIpoBugOrFeature\n");
        return (buf_id.get_data() < 0xffff);
...
template <typename T>
CM_API extern void __write_typed_atomic_impl(SurfaceIndex & buf_id, CmAtomicOpType op,
                                             vector_ref<T, 8> ret, vector<T, 8> src0, vector<T, 8> src1,
                                             vector<uint, 8> u, vector<uint, 8> v, vector<uint, 8> r, vector<uint, 8> LOD)
{
...
template <typename T>
CM_API extern void __write_typed_atomic_impl(SurfaceIndex & buf_id, CmAtomicOpType op,
                                             vector_ref<T, 8> ret, vector<T, 8> src0, int src1,
                                             vector<uint, 8> u, vector<uint, 8> v, vector<uint, 8> r, vector<uint, 8> LOD)
{
    fprintf(stderr, "Error: write typed atomic is not supported in emulation mode.\n");
}
...
template <typename T>
CM_API extern void __write_typed_atomic_impl(SurfaceIndex & buf_id, CmAtomicOpType op,
                                             vector_ref<T, 8> ret, int src0, int src1,
                                             vector<uint, 8> u, vector<uint, 8> v, vector<uint, 8> r, vector<uint, 8> LOD)
{
    fprintf(stderr, "Error: write typed atomic is not supported in emulation mode.\n");
}
...
CM_API extern void __write_typed_atomic_impl(SurfaceIndex & buf_id, CmAtomicOpType op, 
                                             vector_ref<T, 8> ret, vector<T, 8> src0, vector<T, 8> src1,
                                             vector<uint, 8> u, vector<uint, 8> v, vector<uint, 8> r, int LOD)
{
    fprintf(stderr, "Error: write typed atomic is not supported in emulation mode.\n");
}
...
CM_API extern void __write_typed_atomic_impl(SurfaceIndex & buf_id, CmAtomicOpType op, 
                                             vector_ref<T, 8> ret, vector<T, 8> src0, vector<T, 8> src1,
                                             vector<uint, 8> u, vector<uint, 8> v, int r, int LOD)
{
    fprintf(stderr, "Error: write typed atomic is not supported in emulation mode.\n");
}
...
CM_API extern void __write_typed_atomic_impl(SurfaceIndex & buf_id, CmAtomicOpType op, 
                                             vector_ref<T, 8> ret, vector<T, 8> src0, vector<T, 8> src1,
                                             vector<uint, 8> u, int v, int r, int LOD)
{
    fprintf(stderr, "Error: write typed atomic is not supported in emulation mode.\n");
}
...
template<CmAtomicOpType Op, typename T>
_GENX_ inline typename hidden::cm_enable_if<(Op == ATOMIC_INC || Op == ATOMIC_DEC), void>::type
    write_typed_atomic(SurfaceIndex surfIndex, vector_ref<T, 8> ret, vector<uint, 8> u) {
        __write_typed_atomic_impl<T>(surfIndex, Op, ret, NULL, NULL, u, NULL, NULL, NULL);
}

template<CmAtomicOpType Op, typename T>
_GENX_ inline typename hidden::cm_enable_if<(Op == ATOMIC_INC || Op == ATOMIC_DEC), void>::type
    write_typed_atomic(SurfaceIndex surfIndex, vector_ref<T, 8> ret, vector<uint, 8> u, vector<uint, 8> v) {
...
_GENX_ inline typename hidden::cm_enable_if<(Op == ATOMIC_INC || Op == ATOMIC_DEC), void>::type
    write_typed_atomic(SurfaceIndex surfIndex, vector_ref<T, 8> ret, vector<uint, 8> u, vector<uint, 8> v,
                       vector<uint, 8> r, vector<uint, 8> LOD) {
...
_GENX_ inline typename hidden::cm_enable_if<(Op != ATOMIC_CMPXCHG && Op != ATOMIC_INC && Op != ATOMIC_DEC), void>::type
    write_typed_atomic(SurfaceIndex surfIndex, vector_ref<T, 8> ret, vector<T, 8> src0,
                       vector<uint, 8> u) {
...
_GENX_ inline typename hidden::cm_enable_if<(Op != ATOMIC_CMPXCHG && Op != ATOMIC_INC && Op != ATOMIC_DEC), void>::type
    write_typed_atomic(SurfaceIndex surfIndex, vector_ref<T, 8> ret, vector<T, 8> src0,
                       vector<uint, 8> u, vector<uint, 8> v) {
...
_GENX_ inline typename hidden::cm_enable_if<(Op != ATOMIC_CMPXCHG && Op != ATOMIC_INC && Op != ATOMIC_DEC), void>::type
    write_typed_atomic(SurfaceIndex surfIndex, vector_ref<T, 8> ret, vector<T, 8> src0,
                       vector<uint, 8> u, vector<uint, 8> v, vector<uint, 8> r) {
...
template<CmAtomicOpType Op, typename T>
_GENX_ inline typename hidden::cm_enable_if<(Op == ATOMIC_CMPXCHG), void>::type
    write_typed_atomic(SurfaceIndex surfIndex, vector_ref<T, 8> ret, vector<T, 8> src0, vector<T, 8> src1,
                       vector<uint, 8> u) {
