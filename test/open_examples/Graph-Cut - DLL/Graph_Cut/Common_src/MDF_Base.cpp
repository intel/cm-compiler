...
{
	// Create a CM Device
	unsigned int Option = 1;	// Disable scratch space, no need for that, no spill happens in real sense kernels. 
	int result;
	...
	CM_DLL_FILE_VERSION RT_DLL_Version;
	pCmDev->GetRTDllVersion(&RT_DLL_Version);
	printf("Runtime DLL version: %d.%d.%d.%d\n", 
		RT_DLL_Version.wMANVERSION, RT_DLL_Version.wMANREVISION, RT_DLL_Version.wSUBREVISION, RT_DLL_Version.wBUILD_NUMBER);

	CM_DLL_FILE_VERSION JIT_DLL_Version;
	pCmDev->GetJITDllVersion(&JIT_DLL_Version);
	printf("JIT DLL version: %d.%d.%d.%d\n", 
		JIT_DLL_Version.wMANVERSION, JIT_DLL_Version.wMANREVISION, JIT_DLL_Version.wSUBREVISION, JIT_DLL_Version.wBUILD_NUMBER);


	if (Version == CM_4_0)
		InitGPU("Graph_Cut_genx_mdf40.isa");
	else if (Version == CM_5_0)
	    InitGPU("Graph_Cut_genx_mdf50.isa");
#ifdef _MDF6
	else if (Version == CM_6_0 || Version-1 == CM_6_0 )
		InitGPU("Graph_Cut_genx_mdf60.isa");
#endif
	else {
		printf("Unsupported CM version %d\n", Version);
		exit(0);
	}
	...
	pCmDev->GetCaps(CAP_KERNEL_COUNT_PER_TASK, CapValueSize, &KernelCountPerTask);
	pCmDev->GetCaps(CAP_SURFACE2D_COUNT, CapValueSize, &Surface2DCount);
	pCmDev->GetCaps(CAP_SURFACE_COUNT_PER_KERNEL, CapValueSize, &SurfaceCountPerKernel);
	pCmDev->GetCaps(CAP_SAMPLER_COUNT, CapValueSize, &SamplerCount);
	pCmDev->GetCaps(CAP_SAMPLER_COUNT_PER_KERNEL, CapValueSize, &SamplerCountPerKernel);
	pCmDev->GetCaps(CAP_ARG_COUNT_PER_KERNEL, CapValueSize, &ArgCountPerKernel);
	pCmDev->GetCaps(CAP_ARG_SIZE_PER_KERNEL, CapValueSize, &ArgSizePerKernel);
...
/////////////////////////////////////////////////////////////////////////////////////////////
int MDF_Base::AllocGPUSurface(int FrameWidth, int FrameHeight, CM_SURFACE_FORMAT format, CmSurface2D ** pSurf, SurfaceIndex ** pIndex)
{
	int result;
...
/////////////////////////////////////////////////////////////////////////////////////////////
int MDF_Base::AllocGPUSurfaceUP(int FrameWidth, int FrameHeight, CM_SURFACE_FORMAT format, unsigned char * pBuffer, CmSurface2DUP ** pSurf, SurfaceIndex ** pIndex)
{
	int result;
	...
	// Create a 2D surface
    result = pCmDev->CreateSurface2DUP( FrameWidth, FrameHeight, format, pBuffer, pSurf2 );
...
/////////////////////////////////////////////////////////////////////////////////////////////
int MDF_Base::AllocGPUBuffer(unsigned int size, CmBuffer ** pBuffer, SurfaceIndex ** pIndex)
{
	int result;
...
////////////////////////////////////////////////////////////////////////
int MDF_Base::AllocGPUBufferUP(unsigned int size, unsigned char * pSysMem, CmBufferUP ** pBuffer, SurfaceIndex ** pIndex)
{
	int result;
...
#ifdef CM_DX9
/////////////////////////////////////////////////////////////////////////////////////////////
int MDF_Base::ImportDX9Surface(IDirect3DSurface9 * pD3DSurf, CmSurface2D ** pSurf, SurfaceIndex ** pIndex)
{
	int result;
    CmSurface2D * pSurf2 = NULL;
    SurfaceIndex * pIndex2 = NULL;
...
#ifdef CM_DX11
////////////////////////////////////////////////////////////////////////
int MDF_Base::ImportDX11Surface(ID3D11Texture2D * pD3DSurf, CmSurface2D ** pSurf, SurfaceIndex ** pIndex)
{
	int result;
    CmSurface2D * pSurf2 = NULL;
    SurfaceIndex * pIndex2 = NULL;
...
//////////////////////////////////////////////////////////////////////////////////////////
int MDF_Base::AllocatGPUSampler(CmSurface2D * pInputSurf, SurfaceIndex ** pSurfIndex, SamplerIndex ** pSamplerIndex )
{
    int result;
...
/*
//////////////////////////////////////////////////////////////////////////////////////////
int MDF_Base::AllocatGPUSamplerUP(CmSurface2DUP * pInputSurfUP, SurfaceIndex ** pSurfIndex, SamplerIndex ** pSamplerIndex )
{
    int result;
    SurfaceIndex * pSurfIndex2 = NULL;
    SamplerIndex * pSamplerIndex2 = NULL;
