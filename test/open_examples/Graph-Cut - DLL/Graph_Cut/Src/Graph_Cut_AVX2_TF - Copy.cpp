	...
	short * pSouthCapTF)
{
	...
	for (int y = 0; y < Rows; y++) {
		for (int x = 0; x < Cols; x++) {
			...
			// Set excess flow
			w = GetPix(pWeight, Cols, x, y);
			PutPix(pExcessFlowTF, ColsTF, u + BORDER, v + BORDER, w);    // w>0 if a cap from source exists, w<0 if cap from sink exists. ExcessFlow = w.
			PutPix(pHeightTF, ColsTF, u + BORDER, v + BORDER, (HEIGHT_TYPE)0);

			// Set west and east caps
			w = GetPix(pHCap, Cols, x, y);
			if (x == 0) {
				PutPix(pWestCapTF, ColsTF, u + BORDER, v + BORDER, (short)0);
			}
			else if (x < Cols - 1) {
				PutPix(pWestCapTF, ColsTF, u + BORDER, v + BORDER, w);
				PutPix(pEastCapTF, ColsTF, u-1 + BORDER, v-1 + BORDER, w);
			}
			else {    // x = Cols-1
				PutPix(pWestCapTF, ColsTF, u + BORDER, v + BORDER, w);
				PutPix(pEastCapTF, ColsTF, u-1 + BORDER, v-1 + BORDER, w);
				PutPix(pEastCapTF, ColsTF, u + BORDER, v + BORDER, (short)0);
			}

			// Set north and south caps
			w = GetPix(pVCap, Cols, x, y);
			if (y == 0) {
				PutPix(pNorthCapTF, ColsTF, u + BORDER, v + BORDER, (short)0);
			}
			else if (y < Rows - 1) {
				PutPix(pNorthCapTF, ColsTF, u + BORDER, v + BORDER, w);
				PutPix(pSouthCapTF, ColsTF, u + BORDER, v-1 + BORDER, w);
			}
			else {
				PutPix(pNorthCapTF, ColsTF, u + BORDER, v + BORDER, w);
				PutPix(pSouthCapTF, ColsTF, u + BORDER, v-1 + BORDER, w);
				PutPix(pSouthCapTF, ColsTF, u + BORDER, v + BORDER, (short)0);
			}
...
void AVX2ActiveBlockTF16x16(short * pExcessFlow, HEIGHT_TYPE * pHeight, int Rows, int Cols, int RowsTF, int ColsTF, RelabelBlock* pTableBlock, 	active_list &a_list)
{
	...
	for (int y = BORDER; y < (RowsTF-BORDER); y += SIDE_SQUARE) {
		for (int x = BORDER; x < (ColsTF-BORDER); x += SIDE_SQUARE) {
