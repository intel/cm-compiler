...
void CreateBlockMask(short * pExcessFlow, HEIGHT_TYPE * pHeight, int ImgRows, int ImgCols, 
...
//		char fn[128];
//		sprintf(fn, ".\\Output\\_%d_CPU_GR_Height.%dx%d.Y8", iter, Cols, Rows);
//		Dump2File_8bit(fn, pHeight, Rows*Cols);
...
////////////////////////////////////////////////////////////////////////////////
int GetActivePixels(short * pExcessFlow, HEIGHT_TYPE * pHeight, int Rows, int Cols)
{
	int ActivePixels = 0;
...
int CModel_Push_Relabel(short * pExcessFlow, HEIGHT_TYPE * pHeight, short * pWestCap, short * pNorthCap, 
...
//		CreateBlockMask(pExcessFlow, pHeight, Rows, Cols, pBlockMask, BlkRows, BlkCols, BlkWidth, BlkHeight);
        ...
        sprintf(fn, ".\\Output\\_%d_CPU_ExcessFlow1.%dx%d.Y8", iter, sizeof(HEIGHT_TYPE)*Cols, Rows);
        Dump2File(fn, (unsigned char *) pExcessFlow, Rows*Cols*sizeof(HEIGHT_TYPE));

        sprintf(fn, ".\\Output\\_%d_CPU_NorthCap1.%dx%d.Y8", iter, sizeof(HEIGHT_TYPE)*Cols, Rows);
        Dump2File(fn, (unsigned char *) pNorthCap, Rows*Cols*sizeof(HEIGHT_TYPE));

        sprintf(fn, ".\\Output\\_%d_CPU_SouthCap1.%dx%d.Y8", iter, sizeof(HEIGHT_TYPE)*Cols, Rows);
        Dump2File(fn, (unsigned char *) pSouthCap, Rows*Cols*sizeof(HEIGHT_TYPE));
...
#ifdef _DEBUG
        sprintf(fn, ".\\Output\\_%d_CPU_ExcessFlow2.%dx%d.Y8", iter, sizeof(HEIGHT_TYPE)*Cols, Rows);
        Dump2File(fn, (unsigned char *) pExcessFlow, Rows*Cols*sizeof(HEIGHT_TYPE));

		sprintf(fn, ".\\Output\\_%d_CPU_NorthCap2.%dx%d.Y8", iter, sizeof(HEIGHT_TYPE)*Cols, Rows);
        Dump2File(fn, (unsigned char *) pNorthCap, Rows*Cols*sizeof(HEIGHT_TYPE));

        sprintf(fn, ".\\Output\\_%d_CPU_SouthCap2.%dx%d.Y8", iter, sizeof(HEIGHT_TYPE)*Cols, Rows);
        Dump2File(fn, (unsigned char *) pSouthCap, Rows*Cols*sizeof(HEIGHT_TYPE));

        sprintf(fn, ".\\Output\\_%d_CPU_WestCap2.%dx%d.Y8", iter, sizeof(HEIGHT_TYPE)*Cols, Rows);
        Dump2File(fn, (unsigned char *) pWestCap, Rows*Cols*sizeof(HEIGHT_TYPE));

        sprintf(fn, ".\\Output\\_%d_CPU_EastCap2.%dx%d.Y8", iter, sizeof(HEIGHT_TYPE)*Cols, Rows);
        Dump2File(fn, (unsigned char *) pEastCap, Rows*Cols*sizeof(HEIGHT_TYPE));

#endif
		ActivePixels = CModel_GraphCut_Horizontal_Push(pExcessFlow, pHeight, pWestCap, pEastCap, pBlockMask, Rows, Cols, BlkRows, BlkCols, BlkWidth, BlkHeight);

#ifdef _DEBUG
        sprintf(fn, ".\\Output\\_%d_CPU_ExcessFlow3.%dx%d.Y8", iter, sizeof(HEIGHT_TYPE)*Cols, Rows);
        Dump2File(fn, (unsigned char *) pExcessFlow, Rows*Cols*sizeof(HEIGHT_TYPE));

        sprintf(fn, ".\\Output\\_%d_CPU_WestCap3.%dx%d.Y8", iter, sizeof(HEIGHT_TYPE)*Cols, Rows);
        Dump2File(fn, (unsigned char *) pWestCap, Rows*Cols*sizeof(HEIGHT_TYPE));

        sprintf(fn, ".\\Output\\_%d_CPU_EastCap3.%dx%d.Y8", iter, sizeof(HEIGHT_TYPE)*Cols, Rows);
        Dump2File(fn, (unsigned char *) pEastCap, Rows*Cols*sizeof(HEIGHT_TYPE));
