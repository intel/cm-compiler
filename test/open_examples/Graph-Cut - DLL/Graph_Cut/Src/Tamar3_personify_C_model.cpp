...
void CModel_GraphCut_Relabel_org(short * pExcessFlow,
	HEIGHT_TYPE * pHeight,
	short * pWestCap,
	short * pNorthCap,
	short * pEastCap,
...
void CModel_Global_Relabel_org( //float * pExcessFlow, 
	HEIGHT_TYPE * pHeight,
	short * pWestCap,
	short * pNorthCap,
	short * pEastCap,
...
int ActiveBlocks(short * pExcessFlow, HEIGHT_TYPE * pHeight, RelabelBlock* pTableBlock, int Rows, int Cols)
{
	...
	int lastActiveBlock = 0;
	pTableBlock[lastActiveBlock].nextActiveBlock = -1;
	int activePixels, activeBlocks = 0;
	int currentBlock = 0;
...
*/
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void CModel_GraphCut_Vertical_Push(short * pExcessFlow, HEIGHT_TYPE * pHeight, short * pNorthCap, short * pSouthCap, RelabelBlock* pTableBlock, int Rows, int Cols)
{
	HEIGHT_TYPE HEIGHT_MAX = min(Rows * Cols, TYPE_MAX - 1);
	short flow;
	int nPush = 0;
...
void CModel_GraphCut_Vertical_Push_org(short * pExcessFlow,
	HEIGHT_TYPE * pHeight,
	short * pNorthCap,
	short * pSouthCap,
	unsigned char * pBlockMask,
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void CModel_GraphCut_Horizontal_Push(short * pExcessFlow, HEIGHT_TYPE * pHeight, short * pWestCap, short * pEastCap, RelabelBlock* pTableBlock, int Rows, int Cols)
{
	HEIGHT_TYPE HEIGHT_MAX = min(Rows * Cols, TYPE_MAX - 1);
	short flow;
	int rtn = 0;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int CModel_GraphCut_Horizontal_Push_org(short * pExcessFlow,
	HEIGHT_TYPE * pHeight,
	short * pWestCap,
	short * pEastCap,
	unsigned char * pBlockMask,
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void CModel_GraphCut_Push(short * pExcessFlow, HEIGHT_TYPE * pHeight, short * pNorthCap, short * pSouthCap, short * pWestCap, short* pEastCap, RelabelBlock* pTableBlock, int Rows, int Cols)
{
	HEIGHT_TYPE HEIGHT_MAX = min(Rows * Cols, TYPE_MAX - 1);
	short flow;
	int nPush = 0;
	...
	while (indexActiveBlock != -1)
	{
		...
		for (int y = row; y < (row + SIDE_SQUARE); y++)
			for (int x = col; x < (col + SIDE_SQUARE); x++)
			{
				HEIGHT_TYPE currentHeight = GetPix(pHeight, Cols, x, y);
				if (currentHeight < HEIGHT_MAX) {

					// North neighbour (x, y-1)
					short excessFlow = GetPix(pExcessFlow, Cols, x, y);
					HEIGHT_TYPE northHeight = GetPix(pHeight, Cols, x, y - 1);
					if (y > 0) {
						if (excessFlow > 0 && northHeight == (currentHeight - 1)) {
							short northCap = GetPix(pNorthCap, Cols, x, y);
							flow = min(northCap, excessFlow);
							if (flow != 0) {
								AddPix(pExcessFlow, Cols, x, y, (short)-flow);
								AddPix(pExcessFlow, Cols, x, y - 1, flow);
								AddPix(pNorthCap, Cols, x, y, (short)-flow);
		...
		for (int y = row; y < row + SIDE_SQUARE; y += 1) {
			for (int x = col; x < (col + SIDE_SQUARE); x++) {
				curHeight = GetPix(pHeight, Cols, x, y);
				...
				if ((curHeight < HEIGHT_MAX) && (curExcessFlow > 0)) {
					/* West neighbour (x-1, y) */
					if (x > 0) {
						short westHeight = GetPix(pHeight, Cols, x - 1, y);
						if (westHeight == curHeight - 1) {
					...
					/* East neighbour (x+1, y) */
					if (x < Cols - 1) {
						short eastHeight = GetPix(pHeight, Cols, x + 1, y);
						short eastExcessFlow = GetPix(pExcessFlow, Cols, x + 1, y);
						if (curExcessFlow > 0 && eastHeight == curHeight - 1) {
							flow = min(GetPix(pEastCap, Cols, x, y), curExcessFlow);
							eastExcessFlow += flow;
							AddPix(pExcessFlow, Cols, x, y, (short)-flow);
							PutPix(pExcessFlow, Cols, x + 1, y, eastExcessFlow);
							AddPix(pEastCap, Cols, x, y, (short)-flow);
							AddPix(pWestCap, Cols, x + 1, y, flow);
						}
						curExcessFlow = eastExcessFlow;
						curHeight = eastHeight;
					}
				}
			}
		}
...
void CModel_Push_Relabel(short * pExcessFlow, HEIGHT_TYPE * pHeight, short * pWestCap, short * pNorthCap, 
						...
						unsigned char * pOutput, int Rows, int Cols, /*int BlkRows, int BlkCols, int BlkWidth, int BlkHeight,*/ int nRatio)
{
    int iter = 1;
    int ActiveNode;
	int activePixels;
	//CModel_BlockMask(pExcessFlow, pHeight, Rows, Cols, pBlockMask, BlkRows, BlkCols, BlkWidth, BlkHeight);
	//int size = Rows*Cols / (SIDE_SQUARE*SIDE_SQUARE) + 1;
	/*char* BlockMask = (char*)malloc((size - 1)*sizeof(char));
	char fn[128];*/
	/*HEIGHT_TYPE* pCopyHeight = (HEIGHT_TYPE*)malloc(sizeof(HEIGHT_TYPE)*Cols*Rows);
	memcpy(pCopyHeight, pHeight, sizeof(HEIGHT_TYPE)*Cols*Rows);
	short* pCopyExcessFlow = (short*)malloc(sizeof(short)*Cols*Rows);
	memcpy(pCopyExcessFlow, pExcessFlow, sizeof(short)*Cols*Rows);
	short* pCopyNorthCap = (short*)malloc(sizeof(short)*Cols*Rows);
	memcpy(pCopyNorthCap, pNorthCap, sizeof(short)*Cols*Rows);
	short* pCopySouthCap = (short*)malloc(sizeof(short)*Cols*Rows);
	memcpy(pCopySouthCap, pSouthCap, sizeof(short)*Cols*Rows);
	short* pCopyWestCap = (short*)malloc(sizeof(short)*Cols*Rows);
	memcpy(pCopyWestCap, pWestCap, sizeof(short)*Cols*Rows);
	short* pCopyEastCap = (short*)malloc(sizeof(short)*Cols*Rows);
	memcpy(pCopyEastCap, pEastCap, sizeof(short)*Cols*Rows);*/
    do {
#ifdef _DEBUG
		printf("%3d, ", iter);
#endif
		/*int lastBlock = pTableBlock[0].nextActiveBlock;
		for (int i = 1; i < size; i++)
		{
			if (pTableBlock[i].nextActiveBlock != -1)
			{
				BlockMask[i - 1] = -1;
				lastBlock += pTableBlock[i].nextActiveBlock;
			}
			else
			{
				BlockMask[i - 1] = 0;
		...
		if (lastBlock != -1)
		{
			BlockMask[lastBlock-1] = -1;
		}
		sprintf(fn, "Output\\_%d_CPU_BlockMask.%dx%d.Y8", iter, sizeof(char)*Cols, Rows);
		Dump2File(fn, (unsigned char *)BlockMask, (size-1)*sizeof(char));*/
        ...
        if (iter % nRatio) {
			...
			/*CModel_GraphCut_Relabel_org(pExcessFlow, pHeight, pWestCap, pNorthCap, pEastCap, pSouthCap, pBlockMask, Rows, Cols, BlkRows, BlkCols, BlkWidth, BlkHeight);
			if (memcmp(pCopyHeight, pHeight, sizeof(HEIGHT_TYPE)*Cols*Rows) != 0)
			{
				for (int i = 0; i< Rows*Cols; i++)
				{
					if (pCopyHeight[i] != pHeight[i])
					{
						printf("Height not the same %d\n", i);
						exit(1);
					}
				}
			}*/
			...
			//CModel_Global_Relabel_org(pHeight, pWestCap, pNorthCap, pEastCap, pSouthCap, pBlockMask, Rows, Cols, BlkRows, BlkCols, BlkWidth, BlkHeight);
			//CModel_BlockMask(pExcessFlow, pHeight, Rows, Cols, pBlockMask, BlkRows, BlkCols, BlkWidth, BlkHeight);
			/*if (memcmp(pCopyHeight, pHeight, sizeof(HEIGHT_TYPE)*Cols*Rows) != 0)
			{
				printf("Height not the same\n");
				exit(1);
			}*/
        }
#ifdef _DEBUG
		...
		/*if (GetPix(pCopyWestCap, Cols, 131, 69) == GetPix(pWestCap, Cols, 131, 69))
		{
			printf("excess flow is diffrent\n");
		}*/
		...
		/*if (memcmp(pCopyExcessFlow, pExcessFlow, sizeof(short)*Cols*Rows) != 0)
		{
			for (int i = 0; i< Rows*Cols; i++)
			{
				if (pCopyExcessFlow[i] != pExcessFlow[i])
				{
					printf("excess flow is diffrent %d\n", i);

					exit(1);
				}
			}
			printf("ExcessFlow is not the same\n");
		}
		if (memcmp(pCopyWestCap, pWestCap, sizeof(short)*Cols*Rows) != 0)
		{
			for (int i = 0; i< Rows*Cols; i++)
			{
				if (pCopyWestCap[i] != pWestCap[i])
				{
					printf("excess flow is diffrent %d\n", i);
					exit(1);
				}
	...
	/*	if (memcmp(pCopyEastCap, pEastCap, sizeof(short)*Cols*Rows) != 0)
		{
			printf("EastCapacity not the same\n");
			exit(1);
		}*/
		//CModel_GraphCut_Vertical_Push_org(pExcessFlow, pHeight, pWestCap, pEastCap, pBlockMask, Rows, Cols, BlkRows, BlkCols, BlkWidth, BlkHeight);
		//CModel_GraphCut_Vertical_Push_org(pExcessFlow, pHeight, pNorthCap, pSouthCap, pBlockMask, Rows, Cols, BlkRows, BlkCols, BlkWidth, BlkHeight);
		//CModel_GraphCut_Horizontal_Push(pExcessFlow, pHeight, pWestCap, pEastCap, pTableBlock, Rows, Cols);
		//ActiveNode = CModel_GraphCut_Horizontal_Push_org(pExcessFlow, pHeight, pWestCap, pEastCap, pBlockMask, Rows, Cols, BlkRows, BlkCols, BlkWidth, BlkHeight);
		/*if (memcmp(pCopyExcessFlow, pExcessFlow, sizeof(short)*Cols*Rows) != 0)
		{
			for (int i = 0; i< Rows*Cols; i++)
			{
		...
		if (memcmp(pCopyNorthCap, pNorthCap, sizeof(short)*Cols*Rows) != 0)
		{
			printf("NorthCapacity not the same\n");
			exit(1);
		}
		if (memcmp(pCopySouthCap, pSouthCap, sizeof(short)*Cols*Rows) != 0)
		{
			printf("SouthCapacity not the same\n");
			exit(1);
		}
		/*if (memcmp(pCopyExcessFlow, pExcessFlow, sizeof(short)*Cols*Rows) != 0)
		{
		...
		activePixels = AVX2ActiveBlocks(pExcessFlow, pHeight, pTableBlock, Rows, Cols);	
		//CModel_BlockMask(pExcessFlow, pHeight, Rows, Cols, pBlockMask, BlkRows, BlkCols, BlkWidth, BlkHeight);
