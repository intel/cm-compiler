...
/// GenX tool chain
GenX::GenX(const Driver &D, const llvm::Triple &Triple,
             const ArgList &Args) : ToolChain(D, Triple, Args) {
  // ProgramPaths are found relative to compiler executable.
}

Tool *GenX::buildAssembler() const {
  return new tools::GenX::Assemble(*this);
}
...
bool GenX::isPICDefaultForced() const {
  return false;
}

bool GenX::isPICDefault() const {
  return false;
}

bool GenX::isPIEDefault() const {
  return false;
}
