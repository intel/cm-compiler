...
Compilation *Driver::BuildCompilation(ArrayRef<const char *> ArgList) {
  ...
  if (CCCIsCM()) {
    // clang-cm targets Genx.
    // We only change the arch value here - the vendor and os values are the
    // default values, i.e. the triple becomes genx32-pc-win32.
    llvm::Triple T(DefaultTargetTriple);
    T.setArchName("genx32");
    DefaultTargetTriple = T.str();
  }
  ...
  if (FailingCommands.empty()) {
    ...
    for (ArgStringList::const_iterator it = Files.begin(), ie = Files.end();
      ...
      if (!Err.empty()) {
        ...
        size_t I, E;
        // -main-file-name isn't always present - for example if a non-standard
        // assembler is used. In such cases we won't modify the script.
...
static const char *MakeCLOutputFilename(const ArgList &Args, StringRef ArgValue,
  ...
  if (!llvm::sys::path::has_extension(ArgValue) || IsCMMode) {
    ...
    const char *Extension = types::getTypeTempSuffix(FileType, !IsCMMode,
                                                     IsCMMode);
...
const char *Driver::GetNamedOutputPath(Compilation &C,
  ...
  if ((JA.getType() == types::TY_Object || CCCIsCM()) &&
...
const ToolChain &Driver::getToolChain(const ArgList &Args,
  ...
  if (!TC && ((Target.getArch() == llvm::Triple::genx32) ||
              (Target.getArch() == llvm::Triple::genx64))) {
    // The GenX toolchain is independent of OS
    TC = new toolchains::GenX(*this, Target, Args);
  }
  if (!TC) {
    switch (Target.getOS()) {
