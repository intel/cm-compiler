...
void Parser::ParseDirectDeclarator(Declarator &D) {
  ...
  if (getLangOpts().CPlusPlus && D.mayHaveIdentifier()) {
    ...
    // We ignore the special case of an identifier in a template type arg
    // context as we produce a diagnostic for that elsewhere.
...
static bool hasCMGenxAttribute(AttributeList *AttrList) {
  while (AttrList) {
    if (AttrList->isCMGenxAttribute())
      return true;
    AttrList = AttrList->getNext();
  }
  return false;
...
void Parser::ParseFunctionDeclarator(Declarator &D,
  ...
  if (getLangOpts().MdfCM && D.hasAttributes() &&
      hasCMGenxAttribute(D.getDeclSpec().getAttributes().getList()) &&
