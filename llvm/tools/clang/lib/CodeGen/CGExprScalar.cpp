...
public:
  ...
  Value *EmitScalarConversion(Value *Src, QualType SrcTy, QualType DstTy,
                              SourceLocation Loc, bool IsSaturated = false);
...
Value *ScalarExprEmitter::EmitScalarConversion(Value *Src, QualType SrcType,
                                               ...
                                               SourceLocation Loc,
                                               bool IsSaturated) {
  return EmitScalarConversion(Src, SrcType, DstType, Loc, false, IsSaturated);
}
...
Value *ScalarExprEmitter::EmitScalarConversion(Value *Src, QualType SrcType,
  ...
  if (DstType->isCMVectorMatrixType()) {
    ...
    if (llvm::VectorType *SrcVecTy = dyn_cast<llvm::VectorType>(SrcTy)) {
      ...
      if (SrcNumElts == DstNumElts && !IsSaturated) {
      ...
      } else if (SrcNumElts == DstNumElts && IsSaturated) {
        llvm::Function *Fn = 0;
        ...
        if (DstEltTy->isIntegerTy()) {
          if (SrcEltTy->isFloatingPointTy()) {
            ...
            Fn = DstEltType->isUnsignedIntegerType()
                    ? CGF.CGM.getIntrinsic(llvm::Intrinsic::genx_fptoui_sat, Tys)
                    : CGF.CGM.getIntrinsic(llvm::Intrinsic::genx_fptosi_sat, Tys);
          ...
          if (DstEltTy->getPrimitiveSizeInBits()
            ...
            if (DstEltType->isUnsignedIntegerType()) {
              if (SrcEltType->isUnsignedIntegerType())
                Fn = CGF.CGM.getIntrinsic(llvm::Intrinsic::genx_uutrunc_sat, Tys);
              else
                Fn = CGF.CGM.getIntrinsic(llvm::Intrinsic::genx_ustrunc_sat, Tys);
            } else {
              if (SrcEltType->isUnsignedIntegerType())
                Fn = CGF.CGM.getIntrinsic(llvm::Intrinsic::genx_sutrunc_sat, Tys);
              else
                Fn = CGF.CGM.getIntrinsic(llvm::Intrinsic::genx_sstrunc_sat, Tys);
            }
            return Builder.CreateCall(Fn, Src, "conv.sat");
          }
  ...
  case CK_LValueToRValue:
    ...
    if (CE->isSaturated()) {
      ...
      if (Ty->isFPOrFPVectorTy()) {
        ...
        llvm::Function *F = CGF.CGM.getIntrinsic(llvm::Intrinsic::genx_sat, Ty);
