...
static bool ParseCodeGenArgs(CodeGenOptions &Opts, ArgList &Args, InputKind IK,
  ...
  // cm-asm-name specifies the base name to be used for GenX assembly files
  // and has to be embedded as an attribute in the generated LLVM IR, so we
  // need it to be available to CodeGen.
  if (Args.hasArg(OPT_cm_asm_name))
...
void CompilerInvocation::setLangDefaults(LangOptions &Opts, InputKind IK,
  ...
  if (LangStd == LangStandard::lang_mdf_cm) {
    Opts.MdfCM = 1;
    // CM has native half support (the half keyword is
    //enabled later).
...
#include "clang/Frontend/LangStandards.def"
    ...
    if (LangStd == LangStandard::lang_unspecified)
      ...
      case IK_PreprocessedObjC:
        ...
        break;
      case IK_MDF_CM:
        if (!Std.isCPlusPlus11())
          Diags.Report(diag::err_drv_argument_not_allowed_with)
            << A->getAsString(Args) << "MDF CM";
        break;
      case IK_CXX:
      case IK_ObjCXX:
      case IK_PreprocessedCXX:
