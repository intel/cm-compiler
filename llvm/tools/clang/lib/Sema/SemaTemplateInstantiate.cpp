...
static bool DiagnoseUninstantiableTemplate(Sema &S,
  ...
  if (!Complain || (PatternDef && PatternDef->isInvalidDecl())) {
    ...
    // This could be a CM matrix or vector declared with an invalid element type.
    // If so, produce a more specific error diagnostic.
    // TODO: produce a more precise test - e.g. we may want to check for a matrix
    // or vector template in the ActiveTemplateInstantiations.
    if (S.getLangOpts().MdfCM &&
        TSK == TSK_ImplicitInstantiation) {
      SourceLocation PoI = S.ActiveTemplateInstantiations[0].PointOfInstantiation;
      int n = -1;
      if (CXXRecordDecl *Record = dyn_cast<CXXRecordDecl>(S.ActiveTemplateInstantiations[0].Entity)) {
        if (Record->getNameAsString().compare("vector") == 0)
          n = 0;
        if (Record->getNameAsString().compare("matrix") == 0)
          n = 1;

        // TODO we should highlight the invalid type that prompted this diagnostic
        // as well as the name of the declaration.
        if (n >= 0) {
          S.Diag(PoI, diag::err_cm_invalid_element_type) << n;
          Instantiation->setInvalidDecl();
          return true;
        }
      }
