...
bool Sema::CheckFunctionReturnType(QualType T, SourceLocation Loc) {
  ...
  // Functions can return half FP in MDF CM, but no other dialects.
  if (!getLangOpts().MdfCM && T->isHalfType()) {
...
QualType Sema::BuildFunctionType(QualType T,
  ...
  for (unsigned Idx = 0, Cnt = ParamTypes.size(); Idx < Cnt; ++Idx) {
    ...
    if (ParamType->isVoidType()) {
      ...
      Invalid = true;
    } else if (!getLangOpts().MdfCM && ParamType->isHalfType()) {
      // Disallow half FP arguments, except for MDF CM.
