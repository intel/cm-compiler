    ...
    case Type::DependentCMMatrix: {
      ...
      if (const CMMatrixType *MatrixArg = Arg->getAs<CMMatrixType>()) {
        ...
        NonTypeTemplateParmDecl *NRows =
            getDeducedParameterFromExpr(MatrixParam->getNumRowExpr());

        // Perform deduction on the # of columns, if we can.
        NonTypeTemplateParmDecl *NCols =
            getDeducedParameterFromExpr(MatrixParam->getNumColumnExpr());
        ...
        if (NRows) {
          ...
          if (Sema::TemplateDeductionResult Result =
                  DeduceNonTypeTemplateArgument(S, NRows, ArgNRows,
        ...
        if (NCols) {
          ...
          return DeduceNonTypeTemplateArgument(
              S, NCols, ArgNCols, S.Context.IntTy, false, Info, Deduced);
        }
        ...
        if (NRows) {
          if (Sema::TemplateDeductionResult Result =
                  DeduceNonTypeTemplateArgument(
                      S, NRows, MatrixArg->getNumRowExpr(), Info, Deduced))
            return Result;
        }

        if (NCols) {
          return DeduceNonTypeTemplateArgument(
              S, NCols, MatrixArg->getNumColumnExpr(), Info, Deduced);
        }
...
CheckOriginalCallArgDeduction(Sema &S, Sema::OriginalCallArg OriginalArg, 
  ...
  if (S.getLangOpts().MdfCM) {
    if (DeducedA->isCMVectorType() && A->isCMVectorType()) {
      ...
      // Return false if OK.
      return !(S.Context.hasSameUnqualifiedType(VT1->getElementType(),
                                                VT2->getElementType()) &&
    ...
    if (DeducedA->isCMMatrixType() && A->isCMMatrixType()) {
      ...
      // Return false if OK.
      return !(S.Context.hasSameUnqualifiedType(MT1->getElementType(),
                                                MT2->getElementType()) &&
