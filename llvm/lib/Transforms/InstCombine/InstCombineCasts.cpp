...
#else
  // Allow to duplicate cast operations as they will be eliminated by CSE.
  if (!I->hasOneUse() && !isa<CastInst>(I))
    return false;
#endif
...
Instruction *InstCombiner::visitTrunc(TruncInst &CI) {
  ...
  // Transform "trunc (shl Cst, (zext A))" to "shl (trunc Cst), A"
  // when Cst == (trunc Cst). Also true for "lshr".
  if (Src->hasOneUse() &&
      (match(Src, m_Shl(m_ConstantInt(Cst), m_ZExt(m_Value(A)))) ||
       match(Src, m_LShr(m_ConstantInt(Cst), m_ZExt(m_Value(A)))))) {
    Type *Ty = A->getType();
    unsigned ASize = Ty->getPrimitiveSizeInBits();
    if (ASize < DestTy->getPrimitiveSizeInBits())
      Ty = DestTy;
    const APInt &CV = Cst->getValue();
    APInt NewCV = CV.trunc(Ty->getPrimitiveSizeInBits()).zext(CV.getBitWidth());
    if (NewCV == CV) {
      Value *Shift = nullptr;
      if (isa<ShlOperator>(Src))
        Shift = Builder.CreateShl(Builder.CreateTrunc(Cst, Ty),
                                   Builder.CreateTrunc(A, Ty));
      else
        Shift = Builder.CreateLShr(Builder.CreateTrunc(Cst, Ty),
                                    Builder.CreateTrunc(A, Ty));
      Shift->takeName(Src);
      return CastInst::CreateIntegerCast(Shift, Ty, false);
    }
  }
  ...
  // Transform "trunc (shl (zext A), Cst)" -> "shl (zext A), Cst" if it's
  // sizeof(A) + Cst is no greater than sizeof(trunc). Only do this when it's
  // truncated into i16. Even more, if that 'shl' has more than one uses, they
  // are also replaced with a proper 'zext' so that narrow types are perferred.
  if (CI.getType()->isIntegerTy(16) &&
      match(Src, m_Shl(m_ZExt(m_Value(A)), m_ConstantInt(Cst))) &&
      A->hasOneUse()) {
    unsigned DSize = CI.getType()->getPrimitiveSizeInBits();
    unsigned ASize = A->getType()->getPrimitiveSizeInBits();
    if ((ASize + Cst->getZExtValue()) <= DSize) {
      Instruction *Shl = cast<Instruction>(Src);
      BuilderTy IRB(Builder);
      IRB.SetInsertPoint(Shl);
      Value *NewVal = IRB.CreateZExt(A, CI.getType());
      NewVal = IRB.CreateShl(NewVal, Cst->getZExtValue());
      Value *Ext = IRB.CreateZExt(NewVal, Shl->getType());
      replaceInstUsesWith(*Shl, Ext);
      return replaceInstUsesWith(CI, NewVal);
    }
  }
...
Instruction *InstCombiner::visitSIToFP(CastInst &CI) {
  ...
  if (ZExtInst *ZI = dyn_cast<ZExtInst>(Op)) {
    Value *V = ZI->getOperand(0);
    V = Builder.CreateUIToFP(V, CI.getType());
    return replaceInstUsesWith(CI, V);
  }
