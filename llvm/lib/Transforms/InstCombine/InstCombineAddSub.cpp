...
Instruction *InstCombiner::visitAdd(BinaryOperator &I) {
  ...
  //
  // GenX: disable this as it stops us being able to bale a sub
  // constant that is then used as an indirect region offset.
#if 0
...
Instruction *InstCombiner::visitSub(BinaryOperator &I) {
  ...
  // Transform "sub Cst, (zext RHS)" if Cst is proved to be no less than
  // RHS.
  Value *A;
  ConstantInt *Cst;
  // FIXME: Only consider the specific cases where LHS is a constant.
  if (match(Op0, m_ConstantInt(Cst)) && Op1->hasOneUse() &&
      match(Op1, m_ZExt(m_Value(A)))) {

    // Check constant Cst is the maximum unsigned value regard to value A.
    auto isMaxValue = [](ConstantInt *Cst, Value *A) -> bool {
      unsigned ASize = A->getType()->getPrimitiveSizeInBits();
      const APInt &C = Cst->getValue();
      if (C.trunc(ASize).isMaxValue())
        return true;
      Value *A2;
      ConstantInt *ShAmt;
      if (!match(A, m_Shl(m_ZExt(m_Value(A2)), m_ConstantInt(ShAmt))))
        return false;
      unsigned A2Size = A2->getType()->getPrimitiveSizeInBits();
      return (C.lshr(ShAmt->getValue()).trunc(A2Size).isMaxValue());
    };

    if (isMaxValue(Cst, A)) {
      Value *Sub = nullptr;
      Sub = Builder.CreateSub(Builder.CreateTrunc(Cst, A->getType()), A);
      Sub->takeName(&I);
      return CastInst::CreateIntegerCast(Sub, I.getType(), false);
    }
  }
