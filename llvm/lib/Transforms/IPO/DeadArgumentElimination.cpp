...
bool DeadArgumentEliminationPass::RemoveDeadStuffFromFunction(Function *F) {
  ...
  if (RetTy->isVoidTy() || HasLiveReturnedArg) {
    ...
    for (unsigned i = 0; i != RetCount; ++i) {
      // FIXME: Do not eliminate possible EM argument.
      auto isGenXEM = [=]() {
        Type *Ty = getRetComponentType(F, i);
        return Ty->isVectorTy() && Ty->getVectorNumElements() == 32 &&
               Ty->getScalarSizeInBits() == 1;
      };

      RetOrArg Ret = CreateRet(F, i);
      if (isGenXEM() || LiveValues.erase(Ret)) {
