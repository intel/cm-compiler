...
static cl::opt<int> LICMHoistPressureThreshold(
    "LICM-hoisting-pressure-threshold", cl::Hidden, cl::init(2048),
    cl::desc("Threshold for reg-pressure increases that reverts hoising"));
...
static bool hoist(Instruction &I, const DominatorTree *DT, const Loop *CurLoop,
                  const LoopSafetyInfo *SafetyInfo,
                  std::vector<llvm::Instruction*>  &HoistedInsts,
...
static unsigned EstimateLiveOutPressure(BasicBlock *blk)
{
  const DataLayout DL = blk->getParent()->getParent()->getDataLayout();
  unsigned pressure = 0;
  ...
  do {
    ...
    if (inst->isUsedOutsideOfBlock(blk)) {
      pressure += (unsigned)(DL.getTypeAllocSize(inst->getType()));
    }
...
bool LoopInvariantCodeMotion::runOnLoop(
  ...
  if (Preheader) {
    std::vector<llvm::Instruction*> HoistedInsts;
    std::vector<llvm::Instruction*> UndoLocas;
    std::vector<llvm::Instruction*> FoldErase;
    // save state before hoisting
    auto BeforePressure = EstimateLiveOutPressure(Preheader);
    auto BeforeChanged = Changed;
    Changed |= hoistRegion(DT->getNode(L->getHeader()), AA, LI, DT, TLI, L,
                          CurAST, &SafetyInfo, HoistedInsts, UndoLocas, FoldErase, ORE);
    // revert hoisting if necessary
    auto AfterPressure = EstimateLiveOutPressure(Preheader);
    if (AfterPressure > LICMHoistPressureThreshold + BeforePressure) {
      // revert hoisting
      int HS = HoistedInsts.size();
      for (int i = HS - 1; i >= 0; --i) {
        Instruction *InsPt = UndoLocas[i];
        assert(isa<Instruction>(InsPt));
        HoistedInsts[i]->moveBefore(InsPt);
      }
      NumMovedLoads -= NumHoisted;
      NumMovedCalls -= NumHoisted;
      NumHoisted = 0;
      Changed = BeforeChanged;
    }
    int NE = FoldErase.size();
    for (int i = 0; i < NE; ++i)
      FoldErase[i]->eraseFromParent();
  }
    ...
    if (!inSubLoop(BB, CurLoop, LI))
      for (BasicBlock::iterator II = BB->begin(), E = BB->end(); II != E;) {
        ...
        if (I.getOpcode() == Instruction::FDiv &&
          ...
          hoist(*ReciprocalDivisor, DT, CurLoop, SafetyInfo, HoistedInsts, UndoLocas, ORE);
          Changed = true;
          continue;
        }
        ...
        if (CurLoop->hasLoopInvariantOperands(&I) &&
          ...
          Changed |= hoist(I, DT, CurLoop, SafetyInfo, HoistedInsts, UndoLocas, ORE);
      }
  }
...
static bool hoist(Instruction &I, const DominatorTree *DT, const Loop *CurLoop,
  ...
  HoistedInsts.push_back(&I);
  UndoLocas.push_back(I.getNextNode());
