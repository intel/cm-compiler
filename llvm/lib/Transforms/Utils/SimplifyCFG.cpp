...
static bool SinkCommonCodeFromPredecessors(BasicBlock *BB) {
  ...
  auto ProfitableToSinkInstruction = [&](LockstepReverseIterator &LRI) {
    /* GENX BEGIN */
    // Do not sink vector of i1, as this increases flag register pressure.
    for (auto *Inst : *LRI) {
      if (!Inst)
        continue;
      for (unsigned I = 0, E = Inst->getNumOperands(); I < E; ++I) {
        if (Inst->getOperand(I)->getType()->isVectorTy() &&
            Inst->getOperand(I)->getType()->getScalarType()->isIntegerTy(1))
          return false;
      }
    }
    /* GENX END */
...
// __GENX__
  // Do not fold PHI's from simd-cf.
  if (auto EVI = dyn_cast<ExtractValueInst>(IfCond)) {
    auto AggOp = EVI->getAggregateOperand();
    auto CI = dyn_cast<CallInst>(AggOp);
    if (CI && CI->getCalledFunction()) {
      Function *Callee = CI->getCalledFunction();
      if (Callee->getIntrinsicID() == Intrinsic::genx_simdcf_goto ||
          Callee->getIntrinsicID() == Intrinsic::genx_simdcf_join)
        return false;
    }
  }
// __GENX__
