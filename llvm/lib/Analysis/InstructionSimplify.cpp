  ...
  if (isa<ZExtInst>(LHS) || isa<SExtInst>(LHS)) {
    if (auto RHSC = dyn_cast<Constant>(RHS)) {
      if (RHSC->isNullValue()) {
        ...
        if (cast<IntegerType>(
              Input->getType()->getScalarType())->getBitWidth() == 1) {
...
static Value *SimplifyBitCastInst(Value *Op, Type *Ty, const Query &Q, unsigned) {
  if (Constant *C = dyn_cast<Constant>(Op))
    return ConstantFoldInstOperands(Instruction::BitCast, Ty, C, Q.DL, Q.TLI);
  // A bitcast of a bitcast returns the input of the first bitcast if it is
  // of the right type.
  if (auto BCI = dyn_cast<BitCastInst>(Op)) {
...
Value *llvm::SimplifyBitCastInst(Value *Op, Type *Ty, const DataLayout *DL,
                               const TargetLibraryInfo *TLI,
                               const DominatorTree *DT) {
  return ::SimplifyBitCastInst(Op, Ty, Query (DL, TLI, DT), RecursionLimit);
}
...
static Value *SimplifyCall(Value *V, IterTy ArgBegin, IterTy ArgEnd,
  ...
  if (unsigned IID = F->getIntrinsicID()) {
    ...
    if ((unsigned)(IID - Intrinsic::genx_aaaabegin)
      ...
      Ret = SimplifyGenXIntrinsic(IID, FTy->getReturnType(), ArgBegin, ArgEnd);
    else
      Ret =
...
Value *llvm::SimplifyInstruction(Instruction *I, const DataLayout *DL,
  ...
  case Instruction::BitCast:
    Result = SimplifyBitCastInst(I->getOperand(0), I->getType(), DL, TLI, DT);
    break;
  }
