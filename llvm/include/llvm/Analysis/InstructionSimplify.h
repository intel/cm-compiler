...
namespace llvm {
  ...
  /// SimplifyBitCastInst - Given operands for a BitCastInst, see if we can fold
  /// the result.  If not, this returns null.
  Value *SimplifyBitCastInst(Value *Op, Type *Ty, const DataLayout *TD = nullptr,
                           const TargetLibraryInfo *TLI = nullptr,
                           const DominatorTree *DT = nullptr);
  ...
  ///
  /// If this call could not be simplified returns null.
  Value *SimplifyGenXIntrinsic(unsigned IID, Type *RetTy, Use *ArgBegin, Use *ArgEnd);
  Value *SimplifyGenXIntrinsic(unsigned IID, Type *RetTy, Value *const *ArgBegin, Value *const *ArgEnd);
