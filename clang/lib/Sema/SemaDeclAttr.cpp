...
static void handleCMGenxVolatileAttr(Sema &S, Decl *D,
  ...
  if (AL.getNumArgs() == 1) {
    ...
    if (!OffsetExpr->isIntegerConstantExpr(OffsetVal, S.Context)) {
      S.Diag(AL.getLoc(), diag::err_attribute_argument_type)
          << AL << AANT_ArgumentIntegerConstant << OffsetExpr->getSourceRange();
    }
  }

  uint32_t OffsetNum = (uint32_t)OffsetVal.getZExtValue();
  D->addAttr(CMGenxVolatileAttr::Create(S.Context, OffsetNum, AL));
}
...
static void handleCMFloatControlAttr(Sema &S, Decl *D, const ParsedAttr &AL){
  ...
  if (!isa<FunctionDecl>(D)) {
    S.Diag(AL.getLoc(), diag::warn_attribute_wrong_decl_type) << AL << 0;
    return;
  }
  ...
  if (!ModeExpr->isIntegerConstantExpr(ModeVal, S.Context)) {
    S.Diag(AL.getLoc(), diag::err_attribute_argument_type)
        << AL << AANT_ArgumentIntegerConstant << ModeExpr->getSourceRange();
  }

  uint32_t ModeNum = (uint32_t)ModeVal.getZExtValue();

  D->addAttr(CMFloatControlAttr::Create(S.Context, ModeNum, AL));
}
...
static void handleCMGenxSIMTAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(CMGenxSIMTAttr::Create(S.Context, ModeNum, AL));
}

static void handleCMGenxReplicateMask(Sema &S, Decl *D, const ParsedAttr &AL) {
  D->addAttr(
      CMGenxReplicateMaskAttr::Create(S.Context, AL.getArgAsExpr(0), AL));
}
...
static void handleCMGenxNoSIMDPredAttr(Sema &S, Decl *D, const ParsedAttr &AL) {
  ...
  D->addAttr(CMGenxNoSIMDPredAttr::Create(S.Context, AL));
}
...
static void handleCMOpenCLTypeAttr(Sema &S, Decl *D,
  ...
  D->addAttr(CMOpenCLTypeAttr::Create(S.Context, TypeDesc, AL));
}
