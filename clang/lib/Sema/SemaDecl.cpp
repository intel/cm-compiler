...
void Sema::CheckVariableDeclarationType(VarDecl *NewVD) {
  ...
  if (getLangOpts().OpenCL || getLangOpts().MdfCM) {
    ...
    if (NewVD->isFileVarDecl() || NewVD->isStaticLocal() ||
      ...
      } else if (!T->isSamplerT() && !T->isDependentType() &&
...
Sema::ActOnFunctionDeclarator(Scope *S, Declarator &D, DeclContext *DC,
  ...
  if (getLangOpts().MdfCM) {
    ...
    if (IsGenxMain) {
      ...
      if (!FunctionTemplate && !RetTy->isVoidType()) {
        Diag(D.getIdentifierLoc(), diag::err_expected_kernel_void_return_type);
        D.setInvalidType();
      }
    } else if (RetTy->isCMReferenceType()) {
      // For non-kernel functions we don't allow to return vector_ref and
      // matrix_ref values.
