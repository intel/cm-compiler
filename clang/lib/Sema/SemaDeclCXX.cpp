...
Decl *Sema::BuildStaticAssertDeclaration(SourceLocation StaticAssertLoc,
  ...
  if (!AssertExpr->isTypeDependent() && !AssertExpr->isValueDependent() &&
    ...
    if (!Failed && !Cond) {
      ...
      // CM_STATIC_WARNING.
      if (getLangOpts().MdfCM && (MsgBuffer.startswith("\"CM:e:") ||
                                  MsgBuffer.startswith("\"CM:w:"))) {
        // Calculate the length of the message without the prefix and quotes.
        auto Size = MsgBuffer.size() - 7;
        ...
        if (MsgBuffer.startswith("\"CM:e:")) {
          Diag(Loc, diag::err_cm_static_assert) << MsgBuffer.substr(6, Size);
          Failed = true;
        } else {
          Diag(Loc, diag::warn_cm_static_assert) << MsgBuffer.substr(6, Size);
          Failed = false;
        }
