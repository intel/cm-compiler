...
/// global offset) to be read; ::N must be a power of 2 and at most 32. This
/// offset is in bytes. Note 32 may not be hardware-native.
///
...
template <typename T, int N>
CM_NODEBUG CM_INLINE
typename std::enable_if<(sizeof(T) <= 4) && details::isPowerOf2(N, 32),
                        void>::type
...
/// global offset) to be read; ::N must be a power of 2 and at most 32. This
/// offset is in units of elements. 32 may not be hardware-native.
///
/// \param ret the data location to store the return result.
///
/// ::T T can be either char, uchar, short, ushort, int, uint or float.
...
///
/// \param elementByteOffset zero based offset of each element (relative to the
/// global offset) to be written; ::N must be a power of 2 and at most 32. This
/// offset is in units of bytes. 32 may not be hardware-native
///
...
/// enabled channels and N2 must be either 8, 16, or 32. 32 may not be native.
///
/// \param u the x coordinates of the data elements to be read from surface,
/// which
/// must be in unit of pixels. The size N2 must be either 8, 16 or 32. 32 may
/// not be hardware-native.
///
...
typename std::enable_if<
    details::is_fp_or_dword_type<T>::value &&(N2 == 8 || N2 == 16 || N2 == 32)>::type
...
template <typename T, int N>
typename std::enable_if<
    details::is_fp_or_dword_type<T>::value &&(N == 8 || N == 16 || N == 32)>::type
read_typed(SurfaceIndex surfIndex, ChannelMaskType channelMask,
...
/// number of enabled channels and N2 must be either 8, 16, or 32.
/// 32 may not be hardware-native.
///
/// \param u the x coordinates of the data elements to be read from surface,
...
/// in unit of DWords. The size N2 must be either 8, 16 or 32. 32 may not be native.
///
/// The compiler generates code for GenX hardware to perform scattered read from
/// the given offsets. The results are returned in ::m with each enabled channel
/// returned in the next row of ::m. The enabled channels are returned in R, G,
...
template <typename T, int N>
CM_NODEBUG
CM_INLINE typename std::enable_if<(N == 8 || N == 16 || N == 32) && (sizeof(T) == 4)>::type
...
(N == 8 || N == 16 || N == 32) && (sizeof(T) == 1 || sizeof(T) == 2)>::type
cm_slm_read_scaled(uint slmBuffer, vector<uint, N> vAddr, vector_ref<T, N> vDst) {
  typedef typename details::dword_type<T>::type T1;
...
///
/// Write 8, 16 or 32 data elements (byte, word, or dword) given in the vector
...
/// ::T could be of size byte, word or dword. 32 may not be hardware-native.
///
...
///
/// Read 8, 16, or 32 4-element vectors, say {R,G,B,A}, where each element is of size
...
///
/// Write 8, 16, or 32 4-element vectors, say {R,G,B,A}, where each element is of
...
///
template <typename T, int N>
typename std::enable_if<(N == 8 || N == 16 || N == 32) && sizeof(T) == 4, void>::type
...
template <int N>
typename std::enable_if<(N == 8 || N == 16 || N == 32), void>::type
cm_slm_atomic(uint slmBuffer, CmAtomicOpType op, vector<ushort, N> vAddr,
              int dummy);
