// RUN: %cmc -mCM_old_asm_name -mdump_asm -ferror-limit=99 %w 2>&1 | FileCheck %w
...
void test() {
  ...
  int r32 = s1.template iselect;                        // no member iselect in s1
...
// CHECK: iselect.cpp(54,25):  error: no member named 'iselect' in 'S1'
