...
// RUN: %cmc -mCM_printfargs -mCM_old_asm_name -mdump_asm -Qxcm_jit_target=skl %w 2>&1 | FileCheck --check-prefix=SCHED %w
// RUN: rm %W.isa %W_0.visaasm %W_0.asm %W_0.dat

// RUN: %cmc -mCM_printfargs -mCM_old_asm_name -mdump_asm -Qxcm_jit_target=skl -Qxcm_noschedule %w 2>&1 | FileCheck --check-prefix=NOSCHED %w
// RUN: rm %W.isa %W_0.visaasm %W_0.asm %W_0.dat
