...
constexpr unsigned exec_size = 8;
...
constexpr auto N2 = repeat_count * systolic_depth / 2;

auto test_dpasw(vector<src1_t, N1> src1, vector<src2_t, N2> src2) {
  return cm_dpasw<src1_precision, src2_precision, systolic_depth, repeat_count, res_t, src1_t, src2_t, N>(NULL, src1, src2);
}
...
//
// Checks that dpasw is available on supported platforms.
...
// RUN: %cmc -emit-llvm -march=pvc -- %s 2>&1 | FileCheck --check-prefix NOSUPPORT %s
// RUN: %cmc -emit-llvm -march=pvc-vg -- %s 2>&1 | FileCheck --check-prefix NOSUPPORT %s
// RUN: %cmc -emit-llvm -march=lnl-m -- %s 2>&1 | FileCheck --check-prefix NOSUPPORT %s
//
// Checks that dpasw is unavailable on unsupported platforms.
