...
_GENX_MAIN_ void test() {
  ...
  // CHECK: call <16 x i8> @llvm.genx.biased.rounding.hf8.v16i8.v16f16(<16 x half> %{{[^,]+}}, <16 x i8> %{{[^,]+}})
  Dst = cm_srnd_hf8(SrcF, Bias);

  // CHECK: call <16 x i8> @llvm.genx.biased.rounding.hf8.v16i8.v16f16(<16 x half> %{{[^,]+}}, <16 x i8> %{{[^,]+}})
  Dst = cm_srnd_hf8(SrcH, Bias);

  // CHECK: call <16 x i8> @llvm.genx.biased.rounding.hf8.v16i8.v16bf16(<16 x bfloat> %{{[^,]+}}, <16 x i8> %{{[^,]+}})
  Dst = cm_srnd_hf8(SrcBF, Bias);
}
