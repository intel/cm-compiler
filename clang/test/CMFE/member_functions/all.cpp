...
_GENX_MAIN_
void test() {
  unsigned short r1 = m.all;             // missing '()' // expected-error{{expected '('}}
  unsigned short r2 = m.all(;            // missing ')' // expected-error{{expected ')'}}
  unsigned short r3 = m.all(1;           // unexpected expression // expected-error{{expected ')'}}
  unsigned short r4 = m.all(9);          // unexpected expression // expected-error{{expected ')'}}
